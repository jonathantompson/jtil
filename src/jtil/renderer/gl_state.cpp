...
namespace renderer {
  ...
  void GLState::initGLState() {
    ...
    if (gl_state_initialized_) { 
      throw std::wruntime_error("GLState::initGLState() - state already"
  ...
  void GLState::shutdownGLState() {
    std::lock_guard<std::mutex> lock(state_data_mutex_);
    SAFE_DELETE(ht_GLEnum_2_idata_);
    SAFE_DELETE(idata_2_data_);
    gl_state_initialized_ = false;
  }

  bool GLState::queryGLExtension(const char* ext_name) {
    // According to Apple's doc, you need to call glGetString with the version
    // before getting the extensions
    const GLubyte* strVersion = glGetString(GL_VERSION);
    GLint count;
    glGetIntegerv(GL_NUM_EXTENSIONS, &count);
    size_t extNameLen = strlen(ext_name);
    for (int i = 0;  i < count;  i++) {
      const char* cur_extension = (const char*) glGetStringi(GL_EXTENSIONS, i);
      std::cout << cur_extension << std::endl;
      if ((extNameLen == strlen(cur_extension)) && 
        (strcmp(ext_name, cur_extension) == 0)) {
        return true;
      }
    }
    return false;
  ...
  void GLState::glsPolygonMode(const GLenum face, const GLenum mode) {
    ...
    //GLenum cur_mode[2];
    //getGLenum(GL_POLYGON_MODE, cur_mode, 2 * GLTYPE_INT_SIZE, GLTYPE_INT);
  ...
  void GLState::glsClearStencil(const GLint s) {
    if (setGLenum(GL_STENCIL_CLEAR_VALUE, &s, GLTYPE_INT_SIZE, GLTYPE_INT)){
      glClearStencil(s);
      ERROR_CHECK;
    }
  }

  void GLState::glsStencilMaskSeparate(const GLenum face, 
    const GLuint mask) {

    switch (face) {
    case GL_FRONT_AND_BACK:
      glsStencilMaskSeparate(GL_FRONT, mask);
      glsStencilMaskSeparate(GL_BACK, mask);
      break;
    case GL_FRONT:
      if (setGLenum(GL_STENCIL_WRITEMASK, &mask, GLTYPE_INT_SIZE,
        GLTYPE_INT)) {
          glStencilMaskSeparate(GL_FRONT, mask);
      }
      break;
    case GL_BACK:
      if (setGLenum(GL_STENCIL_BACK_WRITEMASK, &mask, GLTYPE_INT_SIZE,
        GLTYPE_INT)) {
          glStencilMaskSeparate(GL_BACK, mask);
      }
      break;
    default:
      throw std::wruntime_error("GLState::glsStencilMaskSeparate() - target"
        " invalid");
    }
    ERROR_CHECK;
  }

  void GLState::glsStencilMask(const GLuint mask) {
    glsStencilMaskSeparate(GL_FRONT_AND_BACK, mask);
  }
  ...
  // USAGE: glsQueryIfTextureIsBound(GL_TEXTURE_2D, GL_TEXTURE0, h_texture);
  bool GLState::glsQueryIfTextureIsBound(const GLenum target, 
    const GLenum target_id, const GLuint texture) {
    GLStateTexture query_enum = GLTarget2GLStateTexture(target);
    int active_tex_index = target_id - GL_TEXTURE0;
    return bound_textures_[active_tex_index][query_enum] == texture;
  }
  ...
  void GLState::glsPatchParameteri(const GLenum pname, const GLint value) {
    if (setGLenum(pname, &value, GLTYPE_INT_SIZE, GLTYPE_INT)) {
