...
#include "jtil/renderer/geometry/mesh_simplification/min_heap_edges.h"
...
namespace mesh_simplification {
  ...
  void MeshSimplification::simplifyMesh(Vector<Float3>& normals_return, 
    Vector<uint32_t>& indices, const uint32_t edge_reduction, 
    Vector<Float3>& vertices, Vector<Float3>& colors) {
  ...
  // The input mesh is LOCALLY MANIFOLD.  This is very important.  There can 
  // be holes in the mesh, and the mesh can have an arbitrary number of 
  // contour edges, but the normals of the faces surrounding a vertex must 
  // not flip.
  void MeshSimplification::facesToWingedEdge(Vector<Edge>& we, 
    Vector<Float3>& normals, const Vector<Float3>& vertices, 
    const Vector<uint32_t>& indices) {
      ...
      for (uint32_t i = 0; i < indices.size(); i+=3) {
        ...
        insertEdges(we, normals, vertices, indices, indices.at(i));
      }
  }

  float MeshSimplification::calcAngleSafe(const Float3& pt0, 
    const Float3& pt1, const Float3& pt2)  {
  ...
  void MeshSimplification::calcNormalsPerVertex(Vector<Float3>& normals,
    const Vector<Float3>& vertices, const Vector<uint32_t>& indices) {
      ...
      // For each face in the index array, calculate its normal and add it to 
      ...
      for (uint32_t i = 0; i < indices.size(); i+=3) {
        ...
        case SimpleNormalApproximation:
          // METHOD 1 --> FAST
          // If you sum the normal weighted by the tri area, then you get a 
          // better average normal.  More importantly, if you calculate the 
          // normal by cross product then you get a normal whos lenght is 2 x 
          // the triangle area, so summing these gives us the correct weights.
          calcNormalUnNormalized(cur_normal, vertices[v1], vertices[v2], 
            vertices[v3]);
          Float3::add(normals[v1], normals[v1], cur_normal);
          Float3::add(normals[v2], normals[v2], cur_normal);
          Float3::add(normals[v3], normals[v3], cur_normal);
          break;
        case RobustNormalApproximation:
          // METHOD 2 --> EXPENSIVE
          // Weight the normals by the angle they form at the vertex.
          float angle = calcNormalAndAngle(cur_normal, vertices[v1], 
            vertices[v2], vertices[v3]);
          norm = normals.at(v2);
          norm->m[0] += cur_normal[0] * angle;
          norm->m[1] += cur_normal[1] * angle;
          norm->m[2] += cur_normal[2] * angle;
          angle = calcAngleSafe(vertices[v3], vertices[v1], vertices[v2]);
          norm = normals.at(v1);
          norm->m[0] += cur_normal[0] * angle;
          norm->m[1] += cur_normal[1] * angle;
          norm->m[2] += cur_normal[2] * angle;
          angle = calcAngleSafe(vertices[v2], vertices[v3], vertices[v1]);
          norm = normals.at(v3);
          norm->m[0] += cur_normal[0] * angle;
          norm->m[1] += cur_normal[1] * angle;
          norm->m[2] += cur_normal[2] * angle;
  ...
  void MeshSimplification::updateVertexNormal(Vector<Float3>& normals, 
    const Vector<Float3>& vertices, const Edge* edge, 
    const uint32_t vertex_to_patch) {
      const Edge* cur_edge = edge;
      Float3 cur_normal;
      do {
        uint32_t v1 = MAX_UINT32;
        uint32_t v2 = MAX_UINT32;
        uint32_t v3 = MAX_UINT32;
        static_cast<void>(v1);
        static_cast<void>(v2);
        static_cast<void>(v3);
        v1 = MAX_UINT32;
        ...
        if (cur_edge->v1 == vertex_to_patch) {
          if (cur_edge->face_a) {
            v1 = cur_edge->v1;
            v2 = cur_edge->v2;
            v3 = (cur_edge->e1a->v1) == v1 ? cur_edge->e1a->v2 : 
              cur_edge->e1a->v1;
          } else if (cur_edge->face_b) {
            v1 = cur_edge->v2;
            v2 = cur_edge->v1;
            v3 = (cur_edge->e2b->v1) == v1 ? cur_edge->e2b->v2 : 
              cur_edge->e2b->v1;
          }
        }
        if (v1 != MAX_UINT32) {
          ...
          case SimpleNormalApproximation:
            calcNormalUnNormalized(cur_normal, vertices[v1], vertices[v2], 
              vertices[v3]);
            Float3::add(normals[vertex_to_patch], normals[vertex_to_patch], 
              cur_normal);
            break;
          case RobustNormalApproximation:
            calcNormal(cur_normal, vertices[v1], vertices[v2], 
              vertices[v3]);
            float angle = calcAngleSafe(vertices[v3], vertices[v1], 
              vertices[v2]);
  ...
  float MeshSimplification::calcEdgeAngle(const Float3& v1, 
    const Float3& v2, const Float3& normal, const Float3& reference_vector) {
      // Project the edge onto the normal
      Float3::sub(tmp1_, v2, v1);  // Vector from v1 -> v2
      ...
      // tmp1_ is now the normalized projection of the vector onto the plane,  
      // now get the angle between tmp1_ and the reference vector
      ...
      // check the magnitude of the dot product to the normal get the sign of 
      // the angle
      Float3::cross(tmp2_, reference_vector, tmp1_);
      if (Float3::dot(tmp2_, normal) < 0) {
        angle = (2.0f*static_cast<float>(M_PI)) - angle;
      }
      // The above routines result in the angle going clockwise around the 
      // vertex I want angle to be anti-clockwise positive so invert
      angle = (2.0f*static_cast<float>(M_PI)) - angle;
      return angle;
  }

  // pt1, pt2 and pt3 must be counter clockwise
  void MeshSimplification::calcNormalUnNormalized(Float3& normal, 
    const Float3& pt0, const Float3& pt1, const Float3& pt2) {
      math::Float3::sub(tmp1_, pt0, pt1);
      math::Float3::sub(tmp2_, pt2, pt1);
      math::Float3::cross(normal, tmp1_, tmp2_);
  }

  // pt1, pt2 and pt3 must be counter clockwise
  float MeshSimplification::calcNormalAndAngle(Float3& normal, 
    const Float3& pt0, const Float3& pt1, const Float3& pt2) {
      math::Float3::sub(tmp1_, pt0, pt1);
      math::Float3::sub(tmp2_, pt2, pt1);
      math::Float3::cross(normal, tmp1_, tmp2_);
  ...
  // pt1, pt2 and pt3 must be counter clockwise
  void MeshSimplification::calcNormal(Float3& normal, const Float3& pt0, 
    const Float3& pt1, const Float3& pt2) {
      math::Float3::sub(tmp1_, pt0, pt1);
      math::Float3::sub(tmp2_, pt2, pt1);
      math::Float3::cross(normal, tmp1_, tmp2_);
  ...
  void MeshSimplification::insertEdges(Vector<Edge>& we, 
    const Vector<Float3>& normals, const Vector<Float3>& vertices, 
    const Vector<uint32_t>& indices, const uint32_t* v123) {
    ...
    Float3 cur_normal;
    calcNormal(cur_normal, vertices[v123[0]], vertices[v123[1]],
      vertices[v123[2]]);

    if (Float3::dot(cur_normal, normals[v123[0]]) < EPSILON ||
      Float3::dot(cur_normal, normals[v123[1]]) < EPSILON ||
      Float3::dot(cur_normal, normals[v123[2]]) < EPSILON) {
        return;
    }

    // First find or create the 3 edges (don't worry about their pointers
    // into the rest of the data structure yet (ie E1a, E1b, E2a, E2b).
    for (uint32_t cur_pair = 0; cur_pair < 3; cur_pair++) {
      uint32_t v1 = v123[cur_pair];
      uint32_t v2 = v123[(cur_pair+1)%3];
      // Find the lowest of the two vertices: not strictly necessary but
      // makes debug easier -> Gives a consistent access patterns when
      // querying edges attached to vertices
    ...
    // this case we can project each edge onto the plane formed by v1 and
    // it's vertex normal (which has been precomputed) and v2 and it's vertex
    // normal and use this to order the vertices
  ...
  // ind_v1 -> first vertex from the perspective of the caller function 
  // (actual edge may be stored as backwards).
  void MeshSimplification::stichEdge(Edge* edge, const uint32_t ind_v1,
    const Vector<Float3>& vertices, const Vector<Float3>& normals) {
  ...
  // angles in the plane
  void MeshSimplification::findInsertionPoint(const uint32_t vindex, 
    const float new_angle, Edge** cur_edge, Edge** prev_edge) {
  ...
  void MeshSimplification::stichForwardEdge(Edge* edge, 
    const uint32_t ind_v1, const Vector<Float3>& vertices, 
    const Vector<Float3>& normals) {
    static_cast<void>(ind_v1);
    const Float3& vertex1 = vertices[edge->v1];
    const Float3& vertex2 = vertices[edge->v2];

    // Note: A forward edge is potentially the first edge for some vertex,
    // so we may have to update the global vertex_edges_ array.  This can
    // never be the case for a backward edge.
    ...
    if (vertex_edges_[edge->v1] == NULL) {
      ...
      Float3::sub(vertex_reference_vectors_[edge->v1], vertex2, vertex1);
      vertex_reference_vectors_[edge->v1].normalize();
    } else {
      // Calculate the edge angle in the vertex plane
      edge->v1_angle = calcEdgeAngle(vertex1, vertex2, normals[edge->v1],
        vertex_reference_vectors_[edge->v1]);
    ...
    if (vertex_edges_[edge->v2] == NULL) {
      ...
      edge->e2b = edge->e2a;
      edge->v2_angle = 0;  // My edge is the reference edge, so angle is 0
      Float3::sub(vertex_reference_vectors_[edge->v2], vertex1, vertex2);
      vertex_reference_vectors_[edge->v2].normalize();
    } else {
      // Calculate the edge angle in the vertex plane
      edge->v2_angle = calcEdgeAngle(vertex2, vertex1, normals[edge->v2],
        vertex_reference_vectors_[edge->v2]);
      // Now insert into the doubly linked list of edges, based on normal
      Edge* prev_edge;
      Edge* cur_edge;
  ...
  void MeshSimplification::wingedEdgeToFaces(Vector<Edge>& we, 
    ...
    // We need to be able to mark if edges have been processed -> we can
    // either destroy the we connectivity (by setting face_a and face_b), or
    // we can reuse existing data in the edge structure.  Since it would be
    // nice to not destroy we --> I use heap_index to indicate that faces
    // have been processed.  It incurres a little overhead, but I think it's
    // OK since wingedEdgeToFaces is usually called on the reduced mesh.
    ...
    for (uint32_t i = 0; i < we.size(); i ++) {
      ...
      if (e0->face_a && !(e0->heap_index & 1)) {
        ...
        Edge* e1 = e0->e2a;
        Edge* e2 = e0->e1a;  // e0, e1 and e2 should be in anticlock order
      ...
      if (e0->face_b && !(e0->heap_index & 2)) {
        ...
        Edge* e1 = e0->e1b;
        Edge* e2 = e0->e2b;  // e0, e1 and e2 should be in anticlock order
        indices.pushBack(e0->v2);
        indices.pushBack(e0->v1);
        if (e1->v1 != e0->v1) {
          indices.pushBack(e1->v1);
  ...
  void MeshSimplification::cullEdges(Vector<Edge>& we, 
    Vector<Float3>& normals, Vector<Float3>& vertices, 
    Vector<Float3>& colors, const uint32_t edge_reduction) {
    ...
    for (uint32_t i = 0; i < edge_reduction && heap.size() > 3; i++) {
      ...
      // Now check for any edges which would cause the edge collapse to
      // create duplicate edges, and disallow those collapses.
      ...
      if (res == EdgeRemovalError) {
        throw wruntime_error(L"simplifyMesh() - ERROR: Edge removal "
          L"failed!");
        return;
      }
  ...
  EdgeRemovalResult MeshSimplification::removeEdge(Edge* edge_to_remove, 
    Vector<Float3>& normals, Vector<Float3>& vertices, 
    Vector<Float3>& colors, MinHeapEdges* heap) {
    Float3 edge_vec;
    ...
    if (settings_.vertex_merge_method == MidpointVertexMerge) {
      Float3::add(new_vertex_pos, vertices[v2], vertices[v1]);
      Float3::scale(new_vertex_pos, 0.5f);
    } else {
      new_vertex_pos.set(vertices[v1]);
    }
    ...
    if (settings_.edge_constraint == EdgeRemovalConstrainNonManifold) {
      ...
      if (num_edges_on_v2 == 3) {
        v2_new = edge_to_remove->e1b->v1 == v1 ? edge_to_remove->e1b->v2 :
          edge_to_remove->e1b->v1;
      v3_new = edge_to_remove->e1a->v1 == v1 ? edge_to_remove->e1a->v2 :
        edge_to_remove->e1a->v1;
      calcNormal(new_normal, new_vertex_pos, vertices[v2_new],
        vertices[v3_new]);
      // Check against v2 normal
      if (Float3::dot(new_normal, normals[v2_new]) <
        MAX_EDGE_REMOVAL_NORMAL_CHANGE_DOT_PROD) {
          return EdgeRemovalDisallowed;
      }
      // Check against v3 normal
      if (Float3::dot(new_normal, normals[v3_new]) <
        MAX_EDGE_REMOVAL_NORMAL_CHANGE_DOT_PROD) {
          return EdgeRemovalDisallowed;
      }
      } else {
        ...
        while (cur_edge != stop_edge) {
          ...
          if (cur_edge->v1 == v2) {  // forward edge around v2
            v2_new = cur_edge->v2;
            v3_new = (cur_edge->e1a->v1 == v2) ? cur_edge->e1a->v2 :
              cur_edge->e1a->v1;
            calcNormal(new_normal, new_vertex_pos, vertices[v2_new],
              vertices[v3_new]);
            // Check against v2 normal
            ...
            v2_new = cur_edge->v1;
            v3_new = (cur_edge->e2b->v1 == v2) ? cur_edge->e2b->v2 :
              cur_edge->e2b->v1;
            calcNormal(new_normal, new_vertex_pos, vertices[v2_new],
              vertices[v3_new]);
            // Check against v2 normal
      ...
      while (cur_edge != stop_edge) {
        ...
        if (cur_edge->v1 == v1) {  // forward edge around v2
          v2_new = cur_edge->v2;
          v3_new = (cur_edge->e1a->v1 == v1) ? cur_edge->e1a->v2 :
            cur_edge->e1a->v1;
          calcNormal(new_normal, new_vertex_pos, vertices[v2_new],
            vertices[v3_new]);
          // Check against v2 normal
          ...
          v2_new = cur_edge->v1;
          v3_new = (cur_edge->e2b->v1 == v1) ? cur_edge->e2b->v2 :
            cur_edge->e2b->v1;
          calcNormal(new_normal, new_vertex_pos, vertices[v2_new],
            vertices[v3_new]);
          // Check against v2 normal
    ...
    if (settings_.edge_constraint == EdgeRemovalConstrainNonManifold) {
      vertices[v1].set(new_vertex_pos);
      // Next make the color the average of the two
      Float3::add(colors[v1], colors[v1], colors[v2]);
      Float3::scale(colors[v1], 0.5f);
    }

    // Smash all old vertices together so that errors are very obvious
    vertices[v2].set(vertices[v2][0], vertices[v2][1],
    ...
    if (num_edges_on_v2 > 3) {
      // Now step around the new vertex updating all attached edges (since
      // vertex moved)
  ...
  void MeshSimplification::patchVertex(Edge* edge_to_remove, 
    Edge* edge_to_replace_anti_clockwise, Edge* edge_to_replace_clockwise,
    const uint32_t vertex_to_patch) {
  ...
  void MeshSimplification::updateEdgeCostTri(Edge* edge, 
    const uint32_t vertex_to_patch, const Vector<Float3>& vertices, 
    const Vector<Float3>& normals,
    MinHeapEdges* heap) {
    static_cast<void>(normals);
  ...
  void MeshSimplification::updateEdgeCosts(Edge* start_edge, 
    const uint32_t vertex_to_patch, const Vector<Float3>& vertices,
    const Vector<Float3>& normals, MinHeapEdges* heap) {
    if (settings_.edge_constraint == EdgeRemovalConstrainNonManifold) {
      // If the triangle area is the cost, then we need to update ALL edges
      // of the affected triangles
      Edge* cur_edge = start_edge;
      do {
        updateEdgeCostTri(start_edge, vertex_to_patch, vertices, normals,
          heap);
        cur_edge = cur_edge->nextAnticlockwiseAround(vertex_to_patch);
      } while (cur_edge != start_edge);
    } else {
      // Otherwise, just modify the cost of the edges attached to the
      // modified vertex if the cost is just edge length
  ...
  bool MeshSimplification::validateWEStructure(const Vector<Edge>& we) 
    const {
    uint32_t we_size = we.size();
    for (uint32_t i = 0; i < we_size; i++) {
      const Edge* cur_edge = we.at(i);
