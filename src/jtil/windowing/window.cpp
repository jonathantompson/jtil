...
namespace windowing {
  ...
  Window::~Window() {
    ...
    renderer::GLState::shutdownGLState();
  ...
  void Window::errorCallback(int error, const char* description) {
    throw std::wruntime_error("GLFW Error: " + std::string(description));
  }
  ...
  void Window::keyboardInputCB(GLFWwindow* wnd, int key, int scancode, 
    ...
    if (g_window_ && g_window_->glfw_window_ != wnd) {
      throw std::wruntime_error("Window::keyboardInputCB() - ERROR: global "
