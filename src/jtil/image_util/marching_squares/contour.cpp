...
namespace marching_squares {
  ...
  Contour::Contour(const Float2& v1, const uint32_t contour_index, 
    const uint32_t curr) {
    this->v1 = v1;
  ...
  void Contour::calcLengthAndAngle(Vector<Contour>* contours) {
    // Vector from this point to the next
    Float2::sub(v, contours->at(next)->v1, v1);  
    // Vector from last point to this
    Float2::sub(w, v1, contours->at(prev)->v1);  
    length = v.length();
    ...
    if (Float3::dot(cur_norm, contour_normal) < 0) {
      angle *= -1.0f;
    }
    // Angle is -pi --> pi
  }
  
  void Contour::calcCost(Vector<Contour>* contours, 
