...
// Code to make SIMD functions more readable

#ifndef SIMDFuncs_h
#define SIMDFuncs_h

#include <xmmintrin.h> // SIMD SSE instructions - SSE instructions are compiler intrinsics, there are no lib-files.

#ifndef DATA_ALIGNMENT
#define DATA_ALIGNMENT 16
#endif

__declspec(align(DATA_ALIGNMENT)) extern unsigned int IEEE754_NAN[4];
__declspec(align(DATA_ALIGNMENT)) extern unsigned int IEEE754_SIGNHIGH[4];
__declspec(align(DATA_ALIGNMENT)) extern unsigned int IEEE754_EPSILON[4];
__declspec(align(DATA_ALIGNMENT)) extern unsigned int IEEE754_EPSILONBIG[4];

/*
-- SIMD FUNCTIONS TO WRITE
	- vector: store / load					(DONE)
	- vector: +, -, *, /					(DONE) --> 9.2x faster than DirectX
	- vector3 dot product					(DONE) --> 1.4x SLOWER than DirectX (only use it if SSE registers are already loaded)
	- vector3 cross product					(DONE) --> 3.8x faster than DirectX
	- SSE register: copy					(DONE) --> Negletable
	- SSE register: duplicate				(DONE) --> Negletable
	- matrix: store / load					(DONE) --> Fast with aligned read / writes
	- matrix: +, -							(DONE) --> 29x  faster than DirectX
	- matrix: multiply (mat4x4,mat4x4)		(DONE) --> 1.3x faster than DirectX, 						NEEDS WORK
	- matrix: multiply (mat3x3,mat3x3)		(DONE) --> 1.9x faster than DirectX							NEEDS WORK
	- matrix: multiply (mat4x4,vec4)		(DONE) --> 4.5x faster than DirectX, 1.8x jonno code		NEEDS WORK
	- matrix: multiply (mat4x4,vec3)		(DONE) --> 5.3x faster than DirectX, 2.2x jonno code		NEEDS WORK
	- matrix: transpose						(DONE) --> 3.1x faster than DirectX
	- matrix: multiply transpose (mat,mat)
	- matrix: multiply transpose (mat,vec)	(DONE) --> 3.3x faster than DirectX, 1.0x Jonno code		DEFINITELY NEEDS WORK (dumb implementation)
	- vector3: abs							(DONE)
	- matrix: abs							(DONE)
	- matrix: abs + epsilon					(DONE) --> 13x faster than DirectX, 
*/
