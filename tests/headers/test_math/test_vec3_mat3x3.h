...
using jtil::math::Vec3;
using jtil::math::Mat3x3;
...
TEST(Vec3_Mat3x3, SymMatEig) {
  ...
  Vec3<double> eigVals;
  jtil::math::SymMatEigenVecs(eigVecs, &eigVals, &M_1);  // O(n^3)
  ...
  for (int i = 0; i < 3; i ++) {
    ...
    for (int j = 0; j < 3; j ++) {
      ...
      jtil::math::Vec3<double>::scale(eigVecs[i], -1);
      if (Vec3<double>::equal(eigVecs[i], eigVecs_expect[j]))
        eig_vec_correct = true;
  ...
  jtil::math::SymMatEigenVals(&eigVals, &M_1);  // O(n^2)
