...
cpplint_args_file_io = '--filter=-build/header_guard,-legal/copyright,-whitespace/end_of_line,-runtime/arrays,-readability/streams'
        ...
        if not re.search( ignore_pattern, cur_dir ) :  
            ...
            for fname in dirList:
                if (re.search( r'.cpp', fname ) or re.search( r'.h', fname )) and not re.search( ignore_pattern, fname ) :
                    ...
                    if not re.search( 'file_io', cur_dir ) :  
                        COMMAND = 'cpplint.py ' + cpplint_args + ' ' + cur_dir + '\\' + fname
                    else :
                        COMMAND = 'cpplint.py ' + cpplint_args_file_io + ' ' + cur_dir + '\\' + fname
                    log = open('log.txt', "w")
