...
namespace mesh_simplification {
  ...
  typedef enum {
    SimpleNormalApproximation,  // ave normals around vert weighted by area
    RobustNormalApproximation,  // ave normals weighted by angle at the vert
  ...
  public:
    ...
    void simplifyMesh(data_str::Vector<math::Float3>& normals_return, 
      data_str::Vector<uint32_t>& indices,
      const uint32_t edge_reduction,
      data_str::Vector<math::Float3>& vertices, 
      data_str::Vector<math::Float3>& colors);

    data_str::Vector<Edge>& getWEStructure() { return we_; }
    void calcNormalsPerVertex(data_str::Vector<math::Float3>& normals_return,
      const data_str::Vector<math::Float3>& vertices, 
      const data_str::Vector<uint32_t>& indices);
  ...
  private:
    ...
    void cullEdges(data_str::Vector<Edge>& we, 
      data_str::Vector<math::Float3>& normals, 
      data_str::Vector<math::Float3>& vertices,
      data_str::Vector<math::Float3>& colors, const uint32_t edge_reduction);
    ...
    void insertEdges(data_str::Vector<Edge>& we, 
      const data_str::Vector<math::Float3>& normals,
      const data_str::Vector<math::Float3>& vertices,  
      const data_str::Vector<uint32_t>& indices, 
      const uint32_t* v123); 
    void calcNormalUnNormalized(math::Float3& normal, const math::Float3&pt0, 
      const math::Float3& pt1, const math::Float3& pt2);
    float calcNormalAndAngle(math::Float3& normal, const math::Float3& pt0, 
      const math::Float3& pt1, const math::Float3& pt2);
    void calcNormal(math::Float3& normal, const math::Float3& pt0, 
      const math::Float3& pt1, const math::Float3& pt2);
    float calcEdgeAngle(const math::Float3& v1, const math::Float3& v2, 
      const math::Float3& normal, const math::Float3& reference_vector);
    void stichEdge(Edge* edge, const uint32_t ind_v1,
      const data_str::Vector<math::Float3>& vertices,
      const data_str::Vector<math::Float3>& normals);
    void stichForwardEdge(Edge* edge, const uint32_t ind_v1,
      const data_str::Vector<math::Float3>& vertices,
      const data_str::Vector<math::Float3>& normals);   
    void findInsertionPoint(const uint32_t vindex, const float angle, 
      Edge** cur_edge, Edge** prev_edge);
    void patchVertex(Edge* edge_to_remove, 
      Edge* edge_to_replace_anti_clockwise, Edge* edge_to_replace_clockwise, 
      const uint32_t vertex_to_patch);
    void updateEdgeCosts(Edge* start_edge, const uint32_t vertex_to_patch,
      const data_str::Vector<math::Float3>& vertices,
      const data_str::Vector<math::Float3>& normals, MinHeapEdges* heap);
    EdgeRemovalResult removeEdge(Edge* edge_to_remove, 
      data_str::Vector<math::Float3>& normals,
      data_str::Vector<math::Float3>& vertices,
      data_str::Vector<math::Float3>& colors, MinHeapEdges* heap);
    float calcAngleSafe(const math::Float3& pt0, const math::Float3& pt1, 
      const math::Float3& pt2);
    void updateEdgeCostTri(Edge* edge, const uint32_t vertex_to_patch, 
      const data_str::Vector<math::Float3>& vertices, 
      const data_str::Vector<math::Float3>& normals, MinHeapEdges* heap);
    void updateVertexNormal(data_str::Vector<math::Float3>& normals,
      const data_str::Vector<math::Float3>& vertices, const Edge* edge, 
      const uint32_t vertex_to_patch);

    bool validateWEStructure(const data_str::Vector<Edge>& we) const;
