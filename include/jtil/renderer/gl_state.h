...
// MAX_DATA_SIZE -> Max data size of uniforms to cache: usually limited by the
//                  maximum number of bones matricies
#define MAX_DATA_SIZE (MAX_BONE_COUNT * 16 * 4)
...
namespace renderer {
  ...
  public:
    ...
    // queryGLExtension - return false if not supported. true if supported
    static bool queryGLExtension(const char* ext_name);
    ...
    static void glsScissor(const GLint x, const GLint y, const GLsizei width, 
      const GLsizei height);
    static void glsClearStencil(const GLint s);
    static void glsStencilMask(const GLuint mask);
    static void glsStencilMaskSeparate(const GLenum face, const GLuint mask);
